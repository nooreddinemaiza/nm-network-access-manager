        ...
        <div x-show="isOpen" x-transition:enter="transition ease-out duration-300"
            ...
            <?= $view->inc('components', 'admin/modal.header.php', [
                'title' => '<svg class="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <h2 class="text-lg font-bold">Statistiques Utilisateurs</h2>',
                'subtitle' => '',
                'close' => 'closeModal()',
            ]); ?>
                            ...
                            <template x-for="(stat, idx) in paginatedStats" :key="`stat-${stat.id}-${idx}`">
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        ...
                                        <!-- Delete -->
                                        <button @click="$store.deleteStats.open(stat,'users')"
...
<!-- Modal Suppression -->
<div x-show="$store.deleteStats.isOpen" x-cloak @keydown.escape.window="$store.deleteStats.close()"
    class="fixed inset-0 z-60 overflow-y-auto" style="display: none;">

    <div class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm" @click="$store.deleteStats.close()"></div>

    <div class="flex min-h-screen items-center justify-center p-4">
        <div x-show="$store.deleteStats.isOpen" x-transition
            class="relative w-full max-w-2xl bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700"
            @click.stop>

            <!-- Header -->
            <div class="bg-gradient-to-r from-red-500 to-rose-600 px-6 py-4 rounded-t-xl">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-bold text-white">Suppression des statistiques</h3>
                    </div>
                    <button @click="$store.deleteStats.close()" class="text-white/80 hover:text-white p-1 rounded-lg hover:bg-white/10 transition-colors">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                        </svg>
                    </button>
            ...
            <div class="p-6 space-y-5" x-show="$store.deleteStats.selectedCible">
                ...
                <div class="flex items-start gap-3 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                    ...
                    <p class="text-xs text-red-700 dark:text-red-400">
                        Cette action est <strong>irréversible</strong>. Les statistiques supprimées ne pourront pas être récupérées.
                    </p>
                </div>

                <!-- Portée de la suppression -->
                <div>
                    <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Portée de la suppression</p>
                    <div class="grid grid-cols-3 gap-2">
                        <button
                            @click="$store.deleteStats.deleteScope = 'all'"
                            :class="$store.deleteStats.deleteScope === 'all'
                                ? 'border-red-500 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'
                                : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-red-300 dark:hover:border-red-700'"
                            class="flex flex-col items-center gap-1.5 p-3 border-2 rounded-xl transition-all text-center">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
                            </svg>
                            <span class="text-xs font-medium leading-tight">Tout supprimer</span>
                        </button>
                        <button
                            @click="$store.deleteStats.deleteScope = 'picker'"
                            :class="$store.deleteStats.deleteScope === 'picker'
                                ? 'border-red-500 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'
                                : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-red-300 dark:hover:border-red-700'"
                            class="flex flex-col items-center gap-1.5 p-3 border-2 rounded-xl transition-all text-center">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <span class="text-xs font-medium leading-tight">Année / Mois / Jour</span>
                        </button>
                        <button
                            @click="$store.deleteStats.deleteScope = 'range'"
                            :class="$store.deleteStats.deleteScope === 'range'
                                ? 'border-red-500 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'
                                : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-red-300 dark:hover:border-red-700'"
                            class="flex flex-col items-center gap-1.5 p-3 border-2 rounded-xl transition-all text-center">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <span class="text-xs font-medium leading-tight">Période personnalisée</span>
                        </button>
                    </div>
                </div>

                <!-- Panel : Tout supprimer -->
                <div x-show="$store.deleteStats.deleteScope === 'all'" x-transition
                    class="p-4 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-xl">
                    <p class="text-sm text-red-700 dark:text-red-400 text-center font-medium">
                        ⚠️ Toutes les statistiques de <strong x-text="$store.deleteStats.selectedCible?.username"></strong> seront définitivement supprimées.
                    </p>
                </div>

                <!-- Panel : Année / Mois / Jour -->
                <div x-show="$store.deleteStats.deleteScope === 'picker'" x-transition class="space-y-3">
                    <div class="flex items-center gap-2">

                        <!-- Année -->
                        <div class="relative flex-1">
                            <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1 ml-0.5">Année</label>
                            <select x-model="$store.deleteStats.deleteYear"
                                class="w-full px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 text-gray-900 dark:text-gray-100 appearance-none pr-8">
                                <option value="all">Toutes</option>
                                <template x-for="year in $store.deleteStats.availableYears" :key="year">
                                    <option :value="year" x-text="year"></option>
                                </template>
                            </select>
                            <svg class="absolute right-2.5 bottom-2 w-3.5 h-3.5 text-gray-400 pointer-events-none" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                            </svg>
                        </div>
                        ...
                        <!-- Mois -->
                        <div class="relative flex-1">
                            <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1 ml-0.5">Mois</label>
                            <select x-model="$store.deleteStats.deleteMonth"
                                :disabled="$store.deleteStats.deleteYear === 'all'"
                                :class="$store.deleteStats.deleteYear === 'all' ? 'opacity-50 cursor-not-allowed' : ''"
                                class="w-full px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 text-gray-900 dark:text-gray-100 appearance-none pr-8 disabled:opacity-50">
                                <option value="all">Tous</option>
                                <option value="1">Janvier</option>
                                <option value="2">Février</option>
                                <option value="3">Mars</option>
                        ...
                        <!-- Jour -->
                        <div class="relative flex-1">
                            <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1 ml-0.5">Jour</label>
                            <select x-model="$store.deleteStats.deleteDay"
                                :disabled="$store.deleteStats.deleteMonth === 'all' || $store.deleteStats.deleteYear === 'all'"
                                :class="$store.deleteStats.deleteMonth === 'all' || $store.deleteStats.deleteYear === 'all' ? 'opacity-50 cursor-not-allowed' : ''"
                                class="w-full px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 text-gray-900 dark:text-gray-100 appearance-none pr-8 disabled:opacity-50">
                                <option value="all">Tous</option>
                                <template x-for="day in $store.deleteStats.deleteDays" :key="day">
                                    <option :value="day" x-text="day"></option>
                                </template>
                            </select>
                            <svg class="absolute right-2.5 bottom-2 w-3.5 h-3.5 text-gray-400 pointer-events-none" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <!-- Résumé picker -->
                    <div x-show="$store.deleteStats.deleteYear !== 'all'" class="text-xs text-gray-500 dark:text-gray-400 px-1">
                        Suppression ciblée :
                        <span class="font-medium text-red-600 dark:text-red-400">
                            <span x-show="$store.deleteStats.deleteDay !== 'all'" x-text="$store.deleteStats.deleteDay + '/'"></span><span x-show="$store.deleteStats.deleteMonth !== 'all'" x-text="$store.deleteStats.deleteMonth + '/'"></span><span x-text="$store.deleteStats.deleteYear"></span>
                        </span>
                    </div>
                </div>

                <!-- Panel : Période personnalisée -->
                <div x-show="$store.deleteStats.deleteScope === 'range'" x-transition class="space-y-3">
                    <div class="flex items-center gap-3">
                        <div class="relative flex-1">
                            <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1 ml-0.5">Du</label>
                            <input type="date" x-model="$store.deleteStats.deleteFrom"
                                class="w-full px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 text-gray-900 dark:text-gray-100">
                        </div>
                        <div class="relative flex-1">
                            <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1 ml-0.5">Au</label>
                            <input type="date" x-model="$store.deleteStats.deleteTo" :min="$store.deleteStats.deleteFrom"
                                class="w-full px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 text-gray-900 dark:text-gray-100">
                        </div>
                    </div>
                    ...
                    <div class="flex gap-2">
                        <span class="text-xs text-gray-500 dark:text-gray-400 self-center">Raccourcis :</span>
                        <button @click="$store.deleteStats.setDeleteRange(7)"
                            :class="$store.deleteStats.isDeleteRange(7) ? 'bg-red-600 text-white border-red-600' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-red-400 hover:text-red-600'"
                            class="px-2.5 py-1 text-xs rounded-lg border transition-colors font-medium">
                            7 jours
                        </button>
                        <button @click="$store.deleteStats.setDeleteRange(30)"
                            :class="$store.deleteStats.isDeleteRange(30) ? 'bg-red-600 text-white border-red-600' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-red-400 hover:text-red-600'"
                            class="px-2.5 py-1 text-xs rounded-lg border transition-colors font-medium">
                            30 jours
                        </button>
                        <button @click="$store.deleteStats.setDeleteRange(90)"
                            :class="$store.deleteStats.isDeleteRange(90) ? 'bg-red-600 text-white border-red-600' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-red-400 hover:text-red-600'"
                            class="px-2.5 py-1 text-xs rounded-lg border transition-colors font-medium">
                            90 jours
                        </button>
                        <button x-show="$store.deleteStats.deleteFrom || $store.deleteStats.deleteTo"
                            @click="$store.deleteStats.deleteFrom = ''; $store.deleteStats.deleteTo = ''"
                            class="ml-auto p-1 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                            </svg>
                        </button>
                    ...
                    <!-- Résumé période -->
                    <div x-show="$store.deleteStats.deleteFrom && $store.deleteStats.deleteTo" class="text-xs text-gray-500 dark:text-gray-400 px-1">
                        Suppression de
                        <span class="font-medium text-red-600 dark:text-red-400" x-text="$store.deleteStats.deleteFrom"></span>
                        au
                        <span class="font-medium text-red-600 dark:text-red-400" x-text="$store.deleteStats.deleteTo"></span>
                    </div>
                </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-900 px-6 py-4 rounded-b-xl border-t border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between gap-3">
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        <span x-show="$store.deleteStats.deleteScope === 'all'">⚠️ Suppression totale sélectionnée</span>
                        <span x-show="$store.deleteStats.deleteScope === 'picker' && $store.deleteStats.deleteYear !== 'all'">📅 Suppression par date sélectionnée</span>
                        <span x-show="$store.deleteStats.deleteScope === 'range' && $store.deleteStats.deleteFrom && $store.deleteStats.deleteTo">📅 Période sélectionnée</span>
                    </p>
                    <div class="flex gap-2">
                        <button @click="$store.deleteStats.close()"
                            class="px-4 py-2 text-sm bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg transition-colors font-medium">
                            Annuler
                        </button>
                        <button @click="$store.deleteStats.confirmDelete()"
                            :disabled="$store.deleteStats.deleteScope === 'picker' && $store.deleteStats.deleteYear === 'all'"
                            :class="$store.deleteStats.deleteScope === 'picker' && $store.deleteStats.deleteYear === 'all'
                                ? 'opacity-50 cursor-not-allowed bg-red-400'
                                : 'hover:bg-red-700'"
                            class="px-4 py-2 text-sm bg-red-600 text-white rounded-lg transition-colors font-medium flex items-center gap-2">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
                            </svg>
                            Supprimer
