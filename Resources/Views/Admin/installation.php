...
.welcome-wrap *  { box-sizing: border-box; }
.welcome-wrap    { font-family: 'Inter', ui-sans-serif, system-ui, sans-serif; color: var(--c-text); }

/* badge pill */
.pill {
    display: inline-flex; align-items: center; gap: .35rem;
    padding: .2rem .65rem; border-radius: 9999px; font-size: .72rem;
    font-weight: 600; letter-spacing: .03em; border: 1px solid;
}
.pill-blue   { background: #1d3461; border-color: #3b82f6; color: #93c5fd; }
.pill-green  { background: #14532d; border-color: #22c55e; color: #86efac; }
.pill-yellow { background: #451a03; border-color: #f59e0b; color: #fcd34d; }
.pill-red    { background: #450a0a; border-color: #ef4444; color: #fca5a5; }
.pill-violet { background: #2e1065; border-color: #8b5cf6; color: #c4b5fd; }
...
.req-card + .req-card { margin-top: 1rem; }
...
/* progress dots */
.step-dots { display: flex; align-items: center; gap: .5rem; }
.step-dot {
                        ...
                        <div class="chk-row">
                            ...
                            <div>
                                ...
                                <div style="display:flex;flex-wrap:wrap;gap:.35rem;margin-top:.5rem;">
                                    <?php foreach ([
                                        'nas','nasreload','radacct','radcheck','radgroupcheck',
                                        'radgroupreply','radpostauth','radreply','radusergroup'
                                    ] as $t): ?>
                        ...
                        <div style="margin-top:1rem;border-top:1px solid #30363d;padding-top:1rem;
                            ...
                            <div style="background:#0d1117;border:1px solid #21262d;border-radius:.6rem;padding:.9rem 1rem;">
                                ...
                                <p style="margin:0 0 .6rem;font-size:.8rem;color:#8b949e;line-height:1.6;">
                                    ...
                                    <span class="ci">/etc/.../mods-enabled/sql</span> :
                                </p>
                            ...
                            <?php elseif ($step == 2): ?>
                                ...
                                function adminConfig() {
                                    return {
                                        ...
                                        showPassword: false, showPasswordConfirm: false, errors: {},
                                        validateField(field) {
                                            let isValid = true;
                                            if (field === 'fullname' && !this.fullname) { this.errors.fullname = "Le nom complet est requis"; isValid = false; }
                                            if (field === 'email' && !this.email) { this.errors.email = "L'email est requis"; isValid = false; }
                                            else if (field === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.email)) { this.errors.email = "Adresse email invalide"; isValid = false; }
                                            if (field === 'password' && !this.password) { this.errors.password = "Le mot de passe est requis"; isValid = false; }
                                            else if (field === 'password' && this.password.length < 8) { this.errors.password = "Minimum 8 caractères"; isValid = false; }
                                            if (field === 'password_confirm' && this.password !== this.passwordConfirm) { this.errors.password_confirm = "Les mots de passe ne correspondent pas"; isValid = false; }
                                            return isValid;
                                        }
                                    }
                                }
...
function formConfig() {
    return {
        ...
        validateField(field) {
            let isValid = true;
            if (field === 'db_host'     && !this.db_host)     { this.errors.db_host     = "L'hôte est requis";               isValid = false; }
            if (field === 'db_port'     && !this.db_port)     { this.errors.db_port     = "Le port est requis";               isValid = false; }
            if (field === 'db_database' && !this.db_database) { this.errors.db_database = "Le nom de la base est requis";     isValid = false; }
            if (field === 'db_username' && !this.db_username) { this.errors.db_username = "Le nom d'utilisateur est requis";  isValid = false; }
            return isValid;
        },
        ...
        validateForm() {
            ...
            this.errors = {};
            <?php if ($step == 1): ?>
            if (!this.db_host)     { this.errors.db_host     = "L'hôte est requis";               isValid = false; }
            if (!this.db_port)     { this.errors.db_port     = "Le port est requis";               isValid = false; }
            if (!this.db_database) { this.errors.db_database = "Le nom de la base est requis";     isValid = false; }
            if (!this.db_username) { this.errors.db_username = "Le nom d'utilisateur est requis";  isValid = false; }
            <?php endif; ?>
            if (isValid) { document.getElementById('configForm').submit(); }
        }
    }
}
</script>
